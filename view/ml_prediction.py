...
from datahandeling_and_other.helper import rank_models,build_model_metrics_line_option,build_prediction_multibar_option,build_actual_vs_prediction_option
import pandas as pd
import numpy as np
...
df = pd.DataFrame({
    ...
    "MAE": [21442.9859, 2590.3825, 6747.5519],
    "MSE": [604938859.4479, 11003764.7349, 75731612.7102],
    "RMSE": [24595.5049, 3317.1923, 8702.3912],
    "R2": [-4.3365, 0.9029, 0.3319]
})

# ------------------ Convert ↓ metrics to ↑ better ------------------
df["MAE_inv"] = 1 / df["MAE"]
df["MSE_inv"] = 1 / df["MSE"]
df["RMSE_inv"] = 1 / df["RMSE"]
df["R2_pos"] = df["R2"]   # R² already higher = better

metrics = ["MAE_inv", "MSE_inv", "RMSE_inv", "R2_pos"]

# ------------------ Min–Max Normalization ------------------
for m in metrics:
    df[m] = (df[m] - df[m].min()) / (df[m].max() - df[m].min())
...
option = {
    "title": {
        "text": "Model Performance Comparison (Normalized Radar)",
        "left": "center"
    },
    "tooltip": {},
    "legend": {
    ...
    "radar": {
        "radius": "65%",
        "indicator": [
            {"name": "MAE (normalized)", "max": 1},
            {"name": "MSE (normalized)", "max": 1},
            {"name": "RMSE (normalized)", "max": 1},
            {"name": "R² (normalized)", "max": 1}
        ]
    },
    "series": [{
        "type": "radar",
...
timeline = ["1 Hour", "1 Day", "1 Week", "1 Month", "3 Months", "6 Months", "1 Year"]

simple_linear = [
    125120.4221, 125221.8275, 125856.713, 128290.4407,
    134639.2958, 144162.5784, 163738.2148
]

multiple_linear = [
    88441.6098, 88445.0077, 88466.2815, 88547.8311,
    88760.569, 89079.6762, 89735.6184
]

polynomial = [
    67723.5511, 66739.6379, 60309.3037, 31128.3785,
    -83593.7069, -385423.4723, -1678069.0588
]
            ...
            "itemStyle": {
                "color": {
                    ...
                    "colorStops": [
                        ...
                        {"offset": 1, "color": "#EE6666"}  # RED for negatives
                    ]
                }
            }
        }
...
actual_prices = [
    122000, 124000, 121500, 113000, 115000, 112000, 108000, 110500,
    109000, 111500, 114000, 113000, 109000, 110500, 102500, 104000,
    103000, 105500, 100000, 95000, 93000, 92000, 90000, 88000,
    91000, 90500, 89000, 94000, 92000, 89500, 90500, 93000,
    92000, 90000, 87000, 88000
]

predicted_poly = np.poly1d(
    np.polyfit(range(len(actual_prices)), actual_prices, deg=2)
)(range(len(actual_prices)))

x_axis = list(range(len(actual_prices)))
...
================================ */
.st-key-nav-btn-2 .stButton button {
    background: linear-gradient(
        135deg,
        var(--primary),
        var(--primary-dark)
...
.st-key-nav-btn-2 .stButton button:hover {
...
def ml_page():
    ...
    for name, key in [
        ...
        metrics = st.session_state["regression_results"][key]["metrics"]
    ...
    model_ranking = rank_models(reg_metrics_rows)

    rows = []

    for tl, preds in st.session_state.get("future_predictions", {}).items():
        rows.append(
            {
                "Timeline": tl,
                "Simple Linear": preds.get("Simple Linear"),
                "Multiple Linear": preds.get("Multiple Linear"),
                "Polynomial": preds.get("Polynomial"),
            }
        )
    ...
    with st.container(key = "kpi-grid"):
        with st.container(key = "kpi-card-1"):
            st.markdown(f"""
                <div class="card-header">
                        <div class="card-title">Best Regression Model</div>
                        <div class="card-description">Model Ranking</div>
                    </div>
                    <div class="card-content less-padding">
                        <div style="display: flex; align-items: center; gap: 1rem; margin-bottom: 1rem;">
                            <span class="badge success">{str(model_ranking.loc[0,"Model"])[:-11]}</span>
                            <div style="flex: 1;">
                                <div style="display: flex; justify-content: flex-end; font-size: 0.875rem;column-gap:1rem;">
                                    <span class="badge neutral">{str(model_ranking.loc[1,"Model"])[:-11]}</span>
                                    <span class="badge neutral">{str(model_ranking.loc[2,"Model"])[:-11]}</span>
                                </div>
                            </div>
                        </div>
        ...
        with st.container(key = "kpi-card-2"):
            st.markdown(f"""
                <div class="kpi-header">
                    <span class="kpi-title">Mean Absolute Error</span>
                ...
                <div class="kpi-value">{round(model_ranking.loc[0,"MAE"],3)}</div>
        ...
        with st.container(key = "kpi-card-3"):
            st.markdown(f"""
                <div class="kpi-header">
                    <span class="kpi-title">Mean Squared Error</span>
                    <svg class="kpi-icon" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M16 7h6v6"></path>
                        <path d="m22 7-8.5 8.5-5-5L2 17"></path>
                    </svg>
                </div>
                <div class="kpi-value">{round(model_ranking.loc[0,"MSE"],3)}</div>
                <div class="kpi-change neutral">
                    <span></span> MSE
                </div>
            """,unsafe_allow_html=True)
        ...
        with st.container(key = "kpi-card-4"):
            st.markdown(f"""
                <div class="kpi-header">
                    <span class="kpi-title">Root Mean Squared Error</span>
                    <svg class="kpi-icon" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                        <path d="M4 14a1 1 0 0 1-.78-1.63l9.9-10.2a.5.5 0 0 1 .86.46l-1.92 6.02A1 1 0 0 0 13 10h7a1 1 0 0 1 .78 1.63l-9.9 10.2a.5.5 0 0 1-.86-.46l1.92-6.02A1 1 0 0 0 11 14z"></path>
                    </svg>
                </div>
                <div class="kpi-value">{round(model_ranking.loc[0,"RMSE"],3)}</div>
                <div class="kpi-change neutral">
                    <span></span> RMSE
                </div>
            """,unsafe_allow_html=True)
        ...
        with st.container(key = "kpi-card-5"):
            st.markdown(f"""
                <div class="kpi-header">
                    <span class="kpi-title">R-squared</span>
                    <svg class="kpi-icon" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                ...
                <div class="kpi-value">{round(model_ranking.loc[0,"R²"],3)}</div>
                <div class="kpi-change neutral">
                    <span></span> R²
                </div>
            """,unsafe_allow_html=True)
        ...
        with st.container(key = "card-1"):
            ...
            with st.container(key = "card-content-1"):
                with st.container(key = "chart-container-1"):
                    st_echarts(build_model_metrics_line_option(model_ranking))
        with st.container(key = "card-2"):
            ...
            with st.container(key = "card-content-2"):
                with st.container(key = "chart-container-2"):
                    st_echarts(build_prediction_multibar_option(preds_df))
    ...
    with st.container(key = "card-3"):
        ...
        with st.container(key = "card-content-3"):
            with st.container(key = "chart-container-large"):
                st_echarts(build_actual_vs_prediction_option(st.session_state["regression_results"]))
                # pass
